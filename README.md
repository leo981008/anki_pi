...
### 使用者系統說明
本應用程式現在包含一個完整的**多使用者系統**。
- **首次註冊:** 第一個註冊的使用者將自動成為**管理員**，並接收所有現有的（無主）資料。
- **後續使用者:** 之後註冊的使用者將擁有各自獨立的資料夾、牌組和卡片。
- **公開牌組:** 使用者可以將自己的牌組設為「公開」，讓其他使用者可以複製這些牌組到自己的帳號下進行學習。
...
- **批次匯入 (檔案):**
    ...
    2.  檔案格式為兩欄或三欄：
        ...
        **不需要標頭**。例如：
        ...
        cat,貓
        ```
    3.  回到主畫面，選擇要匯入的目標牌組，點擊 "📂 從檔案匯入"，系統會自動讀取並匯入 `data.csv` 的內容，然後將其存檔至 `imported_files` 資料夾。

- **批次匯入 (貼上內容):**
    1.  點擊主畫面的 "📋 貼上內容匯入"。
    2.  選擇目標牌組。
    3.  在文字框中直接貼上 CSV 格式的文字（格式同上，不需標頭）。
    4.  點擊匯入即可快速加入大量卡片。
...
- **速記 (Swipe) 模式:**
    - 點擊牌組旁的 "速記" 按鈕。
    - 適合快速、大量的複習。類似 Tinder 的操作，左滑代表 "忘記"，右滑代表 "記得"。
    - 會根據卡片類型調整出題方式 (詳見上方 **卡片類型說明**)。
- **傳統 (Traditional) 模式:**
    - 點擊牌組旁的 "傳統" 按鈕。
    - 傳統的翻卡片模式，提供 "忘記"、"困難"、"普通"、"簡單" 四個選項，對應不同的 SM-2 演算法評分。
    - 會根據卡片類型調整出題方式 (詳見上方 **卡片類型說明**)。
- **AI 隨堂考:**
    - 點擊 "🧠 AI 隨堂考"。
    - 讓 AI (Ollama) 給你出其不意的題目，包含填空造句等，考驗你的真實力。
